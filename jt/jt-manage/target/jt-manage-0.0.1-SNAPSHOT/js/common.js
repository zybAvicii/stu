...
var TT = KindEditorUtil = {		//相当于java中定义的工具类，里面提供了很多静态方法。
	// 编辑器参数
	kingEditorParams : {
		filePostName  : "uploadFile", //<input name="uploadFile">
	...
	formatPrice : function(val,row){
		return (val/100).toFixed(2);//toFixed(2)保留2位小数
	},
	// 格式化商品的状态
    ...
    findItemCatName : function(val,row){
    	var name;
    	//val为cid为商品名称,需要调数据库查询名称
    	$.ajax({
    		type:"post",
    		url:"/item/cat/queryItemName",
    		data:{itemCatId:val},//用户端传给服务器数据
    		cache:true,    //缓存
    		async:false,    //表示同步   默认的是异步的true  
    		//ajax嵌套,2次ajxa都需要异步刷新但是第二次异步刷新不能实现,因为第一次先于第二次刷新,所以嵌套的ajax不能刷新出来,解决办法:设置为同步
